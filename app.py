...
def convert_osmand_to_kmz(uploaded_file, keep_nth_point):
    ...
    if uploaded_file.name.lower().endswith('.gpx'):
        ...
        gpx_files_data.append((uploaded_file.name, uploaded_file.read(), False))
    else:
        with zipfile.ZipFile(uploaded_file, 'r') as z:
            ...
            for gpx_file in gpx_files:
                with z.open(gpx_file) as f:
                    gpx_files_data.append((gpx_file, f.read(), True))

    for gpx_file, xml_content, is_zipped in gpx_files_data:
        try:
